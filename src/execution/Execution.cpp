...
std::unique_ptr<QueryExecutionConfig> createQueryExecutionConfig(execution::ExecutionMode runMode, execution::ExecutionDialect dialect) {
   auto config = std::make_unique<QueryExecutionConfig>();
   if (dialect == ExecutionDialect::SQL) {
      config->frontend = createSQLFrontend();
   } else if (dialect == ExecutionDialect::MLIR) {
      config->frontend = createMLIRFrontend();
   } else if (dialect == ExecutionDialect::PIPEQL) {
      config->frontend = createPipeQLFrontend();
   } else {
      std::cerr << "Invalid dialect" << std::endl;
      exit(1);
   }
   config->queryOptimizer = std::make_unique<DefaultQueryOptimizer>();
   config->loweringSteps.emplace_back(std::make_unique<RelAlgLoweringStep>());
